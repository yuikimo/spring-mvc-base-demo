...
import org.example.multipart.StandardMultipartFile;
...
public class RequestPartMethodArgumentResolver implements HandlerMethodArgumentResolver {
        ...
        for (Part part : parts) {
            if (!ObjectUtils.isEmpty(part)) {
                if (parameterName.equals(part.getName())) {
                    final StandardMultipartFile standardMultipartFile =
                            new StandardMultipartFile(part, part.getSubmittedFileName());
    ...
    private static boolean isMultipartFile(MethodParameter parameter) {
        final Class<?> parameterType = parameter.getParameterType();
