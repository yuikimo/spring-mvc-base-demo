...
import org.example.annotation.ConvertType;
 ...
 * 处理 RequestMapping注解
 */
public class RequestMappingHandlerMethodAdapter extends ApplicationObjectSupport
        implements HandlerMethodAdapter, InitializingBean {
    ...
    // 具体执行的
    @Override
    public void handler(HttpServletRequest req, HttpServletResponse res, HandlerMethod handlerMethod) throws Exception {
    ...
    // 获取WebMvcConfigure中的类型转换器
    public void addConvertMap(Map<Class, ConvertHandler> convertHandlerMap) {
        convertComposite.addConvertMap(convertHandlerMap);
    }
    ...
    public Map<Class, ConvertHandler> getDiyConvertMap() {
        ...
        final String[] names =
                BeanFactoryUtils.beanNamesForAnnotationIncludingAncestors(context, ControllerAdvice.class);
            ...
            for (Method method : methods) {
                if (AnnotatedElementUtils.hasAnnotation(method, ConvertType.class)) {
                    final ConvertType convertType =
                            AnnotatedElementUtils.findMergedAnnotation(method, ConvertType.class);
